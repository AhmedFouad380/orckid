...
function send($tokens,$request,$platform)
{

    if (isset($fcmRegIds)) {
        $SERVER_API_KEY = getServerKey();
...
function send2($tokens, $title = "رسالة جديدة", $msg = "رسالة جديدة فى البريد", $type = 'mail')
{
    ...
    $fields = array
    (
        "registration_ids" => $tokens,  //array of user token whom notification sent two
        "priority" => 10,
        'data' => [
            'title' => $title,
            'body' => $msg,
...
{
    return 'AAAASojRAxc:APA91bGMhgeBz-iLptbbXpz93g2Zy7Wa3X8WyPEB2vVjNU11kKvEMKg7x3ENV8lWntfXvLb1mr-DY9GNhtprldONLtBXLjbk-wOJW4tu02KIPD04ZcXw0rsIG5MjmI9oaIgIZdbZQlhX';
}
...
function upload($file, $dir)
{
 ...
 //   $image = time() . uniqid() . '.' . $file->getClientOriginalExtension();
   ...
   // $path = Storage::disk('s3')->put('uploads' . '/' . $dir, $file);
...
function AwsS3Url(){
    return 'https://dowidar.s3.us-east-2.amazonaws.com/';
}
